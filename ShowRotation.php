...
<?php
  ...
  $day = $_GET["Day"];
  $shift = $_GET["Shift"];
  $schedule = $day."/".$shift."/Schedule.csv";

  // $path_pieces = explode ("/",getcwd());
  //$path_pieces = explode ("/",$_SERVER["SCRIPT_NAME"]);
  //$thisDay = $path_pieces[count($path_pieces)-3];
  //$thisShift = $path_pieces[count($path_pieces)-2];
  $thisDay = $Day[$day];
...
<br> <br>
  <a href="https://photos.app.goo.gl/9BHVpWXa76RESzh58" target=Show> 
    <button> Aquarium Show & Tell </button>
  </a>
